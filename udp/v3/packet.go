...
// HeaderSize 固定 10 字节: Seq(4B) + Ack(4B) + Flag(1B) + SAckCount(1B)
const HeaderSize = 10
...
// Packet 完整数据包
// 物理内存布局: [Header(10B)] + [SAck 数组(SAckCount * 8B)] + [Data(变长，在最末尾)]
type Packet struct {
	Header Header
...
// String 实现 fmt.Stringer 接口，打印美化格式
